...
interface BlindTestQuestionPayload {
  ...
  /** Commentaire/consigne optionnel affiché à côté du lecteur audio (ex: "Soyez précis"). */
  comment: string;
}

function defaultPayload(): BlindTestQuestionPayload {
  return { audioUrl: '', acceptedAnswers: [], expectedAnswers: [], pointsModeOverride: null, comment: '' };
}

@Component({
  selector: 'app-blind-test-question-editor',
  imports: [FormsModule, AudioPlayerComponent, ExpectedAnswersEditorComponent, MediaUploadFieldComponent],
...
export class BlindTestQuestionEditorComponent {
  ...
  protected onAudioUploaded(url: string): void {
    this.payload.update((p) => ({ ...p, audioUrl: url }));
    this.emit();
  }
